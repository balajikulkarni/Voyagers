...
def SelectFrom_DB(sqlite_file,table,lat,lng,boundaries,distance):
    ''' Select entries which fall in boundaries:
        (Lat > Lat_Min and Lat < Lat_Max )
        and
        (Lng > Lng_Min and Lng < Lng_Max )
        and
        (Coordinates Falling within Given Radius) '''
    radius = distance / EARTH_RADIUS
    try:
        con = sql.connect(sqlite_file)
        with con:
            c = con.cursor()
            rc = c.execute('SELECT * FROM {} WHERE \
                          ((LATITUDE => {lt_min}  AND LATITUDE <= {lt_max}) AND (LONGITUDE >= {lg_min} AND LONGITUDE <= {lg_min})) \
					       AND \
					       (acos(sin({lt}) * sin(LATITUDE) + cos({lt}) * cos(LATITUDE) * cos(LONGITUDE-(-{lg}))) <={dist})'
					  .format(table,lt=lat,lt_min=boundaries[0],
					          lt_max=boundaries[1],lg=lng,lg_min=boundaries[2],
					          lg_max=boundaries[3],dist=radius))

            #Proceed Only if we have atleast one valid row.
            if rc >0:
                results = c.fetchall()
                result_dict = [dict((c.description[idx][0],value) for idx,value in enumerate(row)) for row in results]
            else:
                return 'None'
    except Exception:
